...
int diritem_init(diritem_t *item, u8 attr,const char *name) {
    to_sfn((char *)item->DIR_Name, name);
    item->DIR_FstClusHI = (u16 )(FAT_CLUSTER_INVALID >> 16);
    item->DIR_FstClusL0 = (u16 )(FAT_CLUSTER_INVALID & 0xFFFF);
    item->DIR_FileSize = 0;
    item->DIR_Attr = attr;
    item->DIR_NTRes = 0;
...
static int move_file_pos(file_t *file, fat_t *fat, u32 move_bytes, int expand) {
	u32 c_offset = file->pos % fat->cluster_byte_size;
...
int fatfs_mount(struct _fs_t *fs, int major, int minor){
    ...
    if (dev_id < 0) {
        log_print("open disk failed, major: %d, minor: %x", major, minor);
        return -1;
    }

    dbr_t *dbr = (dbr_t *)memory_alloc_page();
    if (!dbr) {
        log_print("mount failed: can not alloc buffer\n");
        goto mount_failed;
    }
    ...
    if (fat->tbl_cnt != 2) {
        log_print("fat table error: major: %x, minor: %x\n", major, minor);
        goto mount_failed;
    }

    if (kernel_strncmp(dbr->BS_FileSysType, "FAT16", 5) != 0) {
        log_print("Not a fat16 file system: major: %x, minor: %x\n", major, minor);
        goto mount_failed;
    }
...
void fatfs_unmount(struct _fs_t *fs){
    ...
    dev_close(fs->dev_id);
    memory_free_page((u32)fat->fat_buffer);
}
...
int fatfs_read (char *buf, int size, file_t *file) {
    ...
    u32 total_read = 0;
    while (nbytes > 0) {
        u32 curr_read = nbytes;
		u32 cluster_offset = file->pos % fat->cluster_byte_size;
        u32 start_sector = fat->data_start + (file->cblk - 2)* fat->sec_per_cluster;  // 从2开始
...
void fatfs_close (file_t *file) {
    ...
    item->DIR_FstClusHI = (u16 )(file->sblk >> 16);
    item->DIR_FstClusL0 = (u16 )(file->sblk & 0xFFFF);
    write_dir_entry(fat, item, file->p_index);
}

int fatfs_seek (file_t *file, u32 offset, int dir) {
     // 只支持基于文件开头的定位
    if (dir != 0) {
        return -1;
    }
    ...
    u32 curr_pos = 0;
    u32 offset_to_move = offset;

    while (offset_to_move > 0) {
        u32 c_off = curr_pos % fat->cluster_byte_size;
        u32 curr_move = offset_to_move;
