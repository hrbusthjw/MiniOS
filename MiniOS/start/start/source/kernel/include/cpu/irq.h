#ifndef IRQ_H
#define IRQ_H

#include "osConfig.h"
#include "common/types.h"
// 异常号宏定义
#define IRQ0_DE 0
#define IRQ1_DB 1
#define IRQ2_NMI 2
#define IRQ3_BP 3
#define IRQ4_OF 4
#define IRQ5_BR 5
#define IRQ6_UD 6
#define IRQ7_NM 7
#define IRQ8_DF 8
#define IRQ10_TS 10
#define IRQ11_NP 11
#define IRQ12_SS 12
#define IRQ13_GP 13
#define IRQ14_PF 14
#define IRQ16_MF 16
#define IRQ17_AC 17
#define IRQ18_MC 18
#define IRQ19_XM 19
#define IRQ20_VE 20
#define IRQ21_CP 21

#define PIC0_ICW1 0X20
#define PIC0_ICW2 0X21
#define PIC0_ICW3 0X21
#define PIC0_ICW4 0X21
#define PIC0_IMR 0X21

#define PIC1_ICW1 0XA0
#define PIC1_ICW2 0XA1
#define PIC1_ICW3 0XA1
#define PIC1_ICW4 0XA1
#define PIC1_IMR 0XA1

#define PIC1_OCW2 0XA0
#define PIC0_OCW2 0X20

#define IRQ0_TIMER      0X20 // 定时器中断IR0
#define IRQ1_KEYBOARD   0X21
#define IRQ14_HADRDISK_PRIMARY  (0X20 + 14)

#define PIC_ICW1_ALWAYS_1 (1 << 4)
#define PIC_ICW1_ICW4 (1 << 0)
#define PIC_ICW4_8086 (1 << 0)

#define PIC_OCW2_EOI (1 << 5) // 一般中断结束方式

#define IRQ_PIC_START 0X20 // PIC中断起始号

// 用于查看page_fault的错误情况
#define ERR_PAGE_P      (1 << 0)
#define ERR_PAGE_WR     (1 << 1)
#define ERR_PAGE_US     (1 << 2)

// 用于查看general protection error的错误情况
#define ERR_EXT         (1 << 0)
#define ERR_IDT         (1 << 1)

typedef struct _exception_frame_t
{
    u32 gs, fs, es, ds;
    u32 edi, esi, ebp, esp, ebx, edx, ecx, eax;
    u32 num, error_code;
    u32 eip, cs, eflags;
    u32 esp3, ss3;
} exception_frame_t;

typedef void (*irq_handler_t)(exception_frame_t *frame);

void irq_init(void);
int irq_install(int irq_num, irq_handler_t handler);
void irq_disable_global(void);
void irq_enable_global(void);
void irq_enable(int irq_num);
void irq_disable(int irq_num);
void irq_send_eoi(int irq_num); // 告诉8259中断处理结束

typedef u32 irq_state_t;
irq_state_t irq_enter_critical_section(void);
void irq_leave_critical_section(irq_state_t state);

void exception_handler_unknown(void);
void exception_handler_divider(void);
void exception_handler_debug(void);
void exception_handler_NMI(void);
void exception_handler_breakpoint(void);
void exception_handler_overflow(void);
void exception_handler_BOUND_Range(void);
void exception_handler_invaild_opcode(void);
void exception_handler_device_unavailbale(void);
void exception_handler_double_fault(void);
void exception_handler_invaild_tss(void);
void exception_handler_segment_not_present(void);
void exception_handler_stack_segmant_fault(void);
void exception_handler_general_protection(void);
void exception_handler_page_fault(void);
void exception_handler_fpu_error(void);
void exception_handler_aligment_check(void);
void exception_handler_machine_check(void);
void exception_handler_simd_exception(void);
void exception_handler_virtual_exception(void);
void exception_handler_control_exception(void);

#endif